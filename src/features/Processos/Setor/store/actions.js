...
const buscarSetores = async ({commit}, dados) => {
    ...
    const request = (await axios.get(process.env.VUE_APP_URL +'/api/setor'+ parametros, config)).data;
...
const buscarFiltro = async ({commit}, dados) => {
    config = {config, ...{headers: {Authorization: `Bearer ` + axios.defaults.headers.common['Authorization'] + '`'}}};

    const request = (await axios.get(process.env.VUE_APP_URL +'/api/setor?limitar=20&buscar='+ dados.buscar, config)).data;
    if (request !== undefined) {
...
const salvar = async ({commit}, dados) => {
    const dadosPost = {
        setor_nome: dados.setor_nome.valor,
        setor_descricao: dados.setor_descricao.valor,
    };

    if (dados.setor_id !== undefined && dados.setor_id.valor !== '') {
        return new Promise((resolve, reject) => {
            config = {config, ...{headers: {Authorization: `Bearer ` + axios.defaults.headers.common['Authorization'] + '`'}}};
            axios.put(process.env.VUE_APP_URL +'/api/setor/'+ dados.setor_id.valor, dadosPost, config)
                .then(resp => {
    ...
    return new Promise((resolve, reject) => {
        config = {config, ...{headers: {Authorization: `Bearer ` + axios.defaults.headers.common['Authorization'] + '`'}}};
        axios.post(process.env.VUE_APP_URL +'/api/setor', dadosPost, config)
            .then(resp => {
                var resposta = helpers.formatarMensagem(resp);
...
const excluir = async ({commit}, dados) => {
    return new Promise((resolve, reject) => {
        config = {config, ...{headers: {Authorization: `Bearer ` + axios.defaults.headers.common['Authorization'] + '`'}}};
        axios.delete(process.env.VUE_APP_URL +'/api/setor/'+ dados.setor_id, config)
            .then(resp => {
                var resposta = helpers.formatarMensagem(resp);
